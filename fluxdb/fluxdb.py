...
class FluxDB:
    ...
    def __init__(self, db_path: str, storage_backend: StorageBackend = None, web: bool = False,
        ...
        self.collection_manager = CollectionManager(db_path, self.index_manager)
        self.transaction_manager = TransactionManager()
        self.buffer_manager = BufferManager(db_path, self.storage, self.buffer_size)
        self.data_manager = DataManager(db_path, self.storage, self.index_manager,
                                       self.buffer_manager, self.transaction_manager)
        os.makedirs(db_path, exist_ok=True)
        os.makedirs(os.path.join(db_path, 'indexes'), exist_ok=True)
        if web:
            self.admin_server = AdminServer(db_path, host, port, debugweb)
